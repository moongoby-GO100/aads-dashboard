...
async def main():
 async with async_playwright() as p:
  ...
  async def websocket(ws):
   ...
   ws.send(json.dumps({'type':'frame','frame':frame,'media_type':'image/png','width':1000,'height':600,'frame_id':'frame-test'}))
   def receive(raw):
    payload=json.loads(raw); calls.append(('ws',payload))
    if payload.get('type')=='control': ws.send(json.dumps({'type':'control_ack','request_id':payload.get('request_id'),'result':{'recipe_step':{'action':'click','selector':'#demo','risk':'READ'}}}))
  ...
  await page.get_by_role('button',name='PC 브라우저',exact=True).click()
  await expect(page.get_by_alt_text('이 채팅 전용 PC 브라우저 실시간 화면')).to_be_visible()
  await page.get_by_alt_text('이 채팅 전용 PC 브라우저 실시간 화면').click(position={'x':100,'y':100})
  await page.get_by_placeholder('선택한 입력칸에 넣을 값').fill('PC input')
  await page.get_by_role('button',name='입력',exact=True).click()
  await page.get_by_role('button',name='Enter',exact=True).click()
  assert any(path=='ws' and data.get('action')=='click' and data.get('frame_id')=='frame-test' for path,data in calls)
  assert any(path=='ws' and data.get('text')=='PC input' and data.get('frame_id')=='frame-test' for path,data in calls)
  assert not any(path=='/pc-agent/execute' for path,data in calls)
  await page.set_viewport_size({'width':390,'height':844})
  await page.evaluate('window.scrollTo(0,0)')
  await page.screenshot(path='/tmp/aads-pc-tab-evidence/mobile-ui.png',full_page=True)
  ...
  await expect(page.get_by_alt_text('이 채팅 전용 PC 브라우저 실시간 화면')).to_have_count(0)
  ...
  print('PASS: server WS click/type/Enter; PC tab WS command/ack without desktop input; recipe start/finish; disconnect disables input; fallback lane isolation')
  await browser.close()
asyncio.run(main())
