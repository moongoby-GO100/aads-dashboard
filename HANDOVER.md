...
## 2026-07-22 11:47 KST - Bulk chat image upload stabilization
- 증상: 채팅 입력창에 이미지 약 30장을 선택해도 메시지 첨부로 반영되지 않고, 동일 파일을 다시 선택하면 서버 저장만 중복되는 현상.
- 실측: 세션 `94a1dc9e-7ba8-4e3f-8f2f-bc83871ed3ee`에서 24개 고유 이미지가 59개 `chat_files` row로 저장됐지만 11:41 KST 사용자 메시지의 `attachments`는 0개였다.
- 원인:
  - `handleFiles()`가 파일을 한 장씩 순차 업로드하면서 `uploading` 상태를 설정하지 않아 업로드 도중 전송 버튼이 활성 상태였다.
  - 전송 시 사전 업로드된 `file_id` 첨부 참조 대신 원본 파일을 multipart로 다시 보내 저장·전송 경로가 이중화됐다.
  - 업로드 진행률과 동일 파일 중복 선택 방지가 없어 사용자가 반영 여부를 알 수 없었다.
- 반영:
  - 최대 50개 첨부, 4개 동시 업로드, `완료/전체/실패` 진행률을 적용하고 업로드 중 전송·삭제·추가 선택을 차단했다.
  - 파일명·크기·수정시각 기준으로 현재 대기 목록의 중복 선택을 제거했다.
...
- 운영 반영·검증 (2026-07-22 11:47~11:56 KST):
  ...
  - 기존 업로드 복구: 사용자 메시지 `1cefd287-0abf-49aa-b3b2-c9aff9e30991`에 고유 이미지 24개를 첨부하고 선택된 `chat_files.message_id` 24건을 연결했다. 검증값은 `attachment_count=24`, `linked_file_count=24`다. 중복 저장 행은 삭제하지 않았다.
