...
- 운영 반영·검증:
  - 커밋 `69df909d0dfc`를 `feat/unni-naengmyeon-homepage-20260722`에 push하고 blue-green 무중단 배포했다. 18:49:51 KST에 blue(3100)로 전환했으며, blue 활성·green standby 모두 같은 release SHA와 healthy 상태를 확인했다.
  - `https://unni.newtalk.kr/unni-naengmyeon/brand/banners`는 HTTP 200이며 B-2 이름과 앞·뒷면 파일 링크를 반환한다. 300DPI 앞면·뒷면도 각각 HTTP 200(33,478,315 bytes / 29,097,170 bytes)이다.
  - 공개 Playwright에서 B-2 전면 카드를 직접 캡처해 냉면 사진·상호·픽업존이 모두 보이는 것을 확인했다. 배포 자동 QA는 `UNKNOWN`이므로 통과로 간주하지 않고 HTTP·컨테이너·브라우저 검증으로 대체했다.
...
## 2026-07-23 13:31 KST - 실내 픽업존 P4 나이트 라이더 B-1 추가

- CEO 요청: 기존 실외 B-1과 같은 느낌의 실내 600×600mm 유리 부착용 배너 시안을 1안 더 제작한다.
- 변경: 배너 페이지에 `INDOOR P4 · 나이트 라이더 B-1`을 추가했다. 실외 B-1의 딥그린·화이트·코랄 고대비와 동일 배경을 재사용하고, 방향 화살표 없이 `언니냉면`, `배달·포장 픽업존`, `주문번호 확인 후 픽업`을 크게 배치했다.
- 출력물: `public/brands/unni-naengmyeon/banners-20260722/print/indoor-p4.png` (600×600mm 비율의 1,200×1,200px RGB PNG).
...
## 2026-07-23 - 실내 유리 P6·P7 냉면 사진형 추가 시안

- CEO 요청: 기존 INDOOR P5를 단순 반영하는 것이 아니라, 냉면 사진이 보이는 수정·추가 시안을 실제 인쇄용으로 제작한다.
- 변경:
  - `INDOOR P6 · 나이트 라이더 B-2 냉면 스포트라이트`: 실제 물냉면을 하단에 크게 배치해 음식 인지를 우선한 사진형 안이다.
  - `INDOOR P7 · 나이트 라이더 B-3 볼드 픽업`: 오른쪽 물냉면 사진과 왼쪽 고대비 상호·픽업 안내를 분리해, 배달기사 동선 인지를 우선한 안이다.
  - P5·P6·P7 모두 `언니냉면 / 배달·포장 픽업존 / 주문번호 확인 후 픽업` 문구, 4개 타공 안전영역 및 일반 PNG·300DPI 아트워크·별도 타공 가이드 다운로드를 제공한다.
  - `scripts/render-unni-indoor-p5-print.mjs`는 `UNNI_INDOOR_IDS`로 대상 안을 선택해 출력할 수 있고, 300DPI 원본과 1,200px 웹 미리보기를 함께 생성한다.
...
## 2026-07-24 KST - INDOOR P6 B-1 냉면 사진 동기화

- CEO 요청: `나이트 라이더 B-2 · 냉면 스포트라이트`의 냉면 사진을 `나이트 라이더 B-1 · 글라스 픽업(P5)`과 동일한 사진으로 통일하고, 인쇄 다운로드 규격을 재확인한다.
- 변경: P6가 전용 `hero-naengmyeon-b1-pickup.webp`를 사용하도록 분리했다. 이 파일은 P5에 사용한 물냉면 사진을 그대로 유지한 웹 최적화본이며, P6의 사진 크롭은 냉면 그릇과 면발이 중앙 하단에 선명하게 보이도록 기존 스포트라이트 구도를 유지한다.

## 2026-07-24 11:10 KST - INDOOR P6 사진 원본을 B-1과 완전 통일
- 검수에서 P5와 P6의 사진 파일 해시가 달라, 기존의 “같은 사진” 설명과 실제 참조가 일치하지 않음을 확인했다.
- `src/app/unni-naengmyeon/brand/banners/page.tsx`: P6가 P5와 동일한 `hero-naengmyeon-brass-v2.webp`를 직접 참조하도록 교정했다. P6 인쇄 원본과 타공 가이드는 이 변경 후 재출력한다.

## 2026-07-24 11:22 KST - INDOOR P4 300DPI·타공 다운로드 보완
- 검수에서 원래 P4는 1,200×1,200px 웹 PNG만 제공하고 있어, P5~P7과 달리 인쇄 원본·타공 가이드 다운로드가 없음을 확인했다.
- P4도 600×600mm, 7,087×7,087px, 300DPI PNG와 4개 Ø10mm(중심선 25mm) 타공 가이드로 출력하고, 배너 페이지에서 두 파일을 내려받을 수 있게 추가한다.
- 인쇄 파일: 기존 공개 다운로드 `indoor-p6-glass-pickup-300dpi.png` 및 타공 가이드는 각각 7,087×7,087px, 300DPI, sRGB PNG(600×600mm)임을 Sharp 메타데이터로 재확인했다. 타공 기준은 4곳 Ø10mm, 각 모서리 중심선 25mm, 중요 정보 사방 35mm 안쪽이다.
- 유의: 사진은 기존 B-1 웹 원본을 인쇄 아트워크에 적용한 것이며, 신규 촬영 원본으로 교체한 것은 아니다. 대형 근접 인쇄 품질이 필요하면 원본 촬영본을 받아 재출력해야 한다.

## 2026-07-24 14:36 KST - INDOOR P4 로고 박스 중앙 정렬 보정
- CEO 요청: `INDOOR P4 · 나이트 라이더 B-1` 인쇄용 이미지의 흰 로고 박스 안에서 검정 로고마크가 하단으로 내려와 보여, 로고 정렬만 즉시 수정한다.
