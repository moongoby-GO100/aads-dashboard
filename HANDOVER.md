...
## 2026-06-12 13:26 KST - Admin user session audit UI and faster admin navigation
...
- 조치:
  ...
  - `src/middleware.ts`에서 메뉴 이동마다 서버 측 `/auth/me`를 재호출하던 관리자 판정을 제거했다. 일반 사용자 차단은 `ClientLayout`과 백엔드 admin API 권한으로 유지한다.
  - `src/lib/auth.ts`에서 `/auth/me` 결과를 60초 캐시해 클라이언트 라우트 이동 시 중복 인증 왕복을 줄였다.
- 검증:
  - `npx eslint src/app/admin/users/page.tsx src/app/admin/sessions/page.tsx src/lib/auth.ts src/middleware.ts` 통과.
  - `npx tsc --noEmit --pretty false` 통과.
  - 전체 `npm run lint`는 기존 전역 lint 부채 때문에 실패한다. 이번 변경 대상 파일 단독 lint는 통과했다.
- 주의:
