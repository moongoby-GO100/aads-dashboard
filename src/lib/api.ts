...
function handle401Redirect(): void {
  ...
  // Preserve hash-based chat session IDs across an expired-login round trip.
...
export const api = {
  ...
  getCollectorSites: (projectKey?: string) =>
    request<{ sites: CollectorSite[]; count: number }>(
      `/authenticated-site-collector/site-profiles${projectKey ? `?project_key=${encodeURIComponent(projectKey)}` : ""}`,
    ),
  getCollectorJobs: (projectKey?: string, status?: string) => {
    const query = new URLSearchParams();
    if (projectKey) query.set("project_key", projectKey);
    if (status) query.set("status", status);
    return request<{ jobs: CollectorJob[]; count: number }>(
      `/authenticated-site-collector/jobs${query.size ? `?${query}` : ""}`,
    );
  },
  createCollectorJob: (data: { project_key: string; site_key: string; recipe_id: string; recipe_version?: string; work_key?: string }) =>
    request<{ status: string; job: CollectorJob }>("/authenticated-site-collector/jobs", {
      method: "POST",
  ...
  dryRunCollectorRecipe: (recipeId: string, version = "v1", targetUrl = "") =>
    request<unknown>(
      `/authenticated-site-collector/recipes/${encodeURIComponent(recipeId)}/dry-run?version=${encodeURIComponent(version)}`,
  ...
  upsertGovernanceIntentPolicy: (data: Record<string, unknown>) =>
    request<any>("/governance/intent-policies", { method: "POST", body: JSON.stringify(data) }),
