...
function trimUrlTrailingPunctuation(value: string): { url: string; trailing: string } {
  let url = value;
  let trailing = "";
  while (/[.,;:!?]$/.test(url)) {
...
function renderPlainWithBareUrls(text: string, keyPrefix = ""): React.ReactNode {
  ...
  while ((match = BARE_URL_RE.exec(text)) !== null) {
    if (match.index > last) parts.push(<span key={`${keyPrefix}t${idx++}`}>{text.slice(last, match.index)}</span>);
    const { url, trailing } = trimUrlTrailingPunctuation(match[1]);
    ...
    if (trailing) parts.push(<span key={`${keyPrefix}p${idx++}`}>{trailing}</span>);
...
function renderPlainWithLinks(text: string, keyPrefix = ""): React.ReactNode {
  const parts: React.ReactNode[] = [];
  let last = 0;
  let idx = 0;
  let match: RegExpExecArray | null;
  LOCAL_FILE_PATH_RE.lastIndex = 0;
  while ((match = LOCAL_FILE_PATH_RE.exec(text)) !== null) {
    if (match.index > last) {
      parts.push(<React.Fragment key={`${keyPrefix}t${idx++}`}>{renderPlainWithBareUrls(text.slice(last, match.index), `${keyPrefix}u${idx}`)}</React.Fragment>);
    }
    const { path, trailing } = trimLocalFileTrailingPunctuation(match[1]);
    parts.push(
      <LocalFileArtifactLink key={`${keyPrefix}f${idx++}`} href={path}>
        {normalizeLocalFileHref(path).split("/").pop() || "서버 파일"}
      </LocalFileArtifactLink>
    );
    if (trailing) parts.push(<span key={`${keyPrefix}p${idx++}`}>{trailing}</span>);
    last = match.index + match[1].length;
  }
  if (last < text.length) parts.push(<React.Fragment key={`${keyPrefix}t${idx++}`}>{renderPlainWithBareUrls(text.slice(last), `${keyPrefix}u${idx}`)}</React.Fragment>);
  return parts.length ? parts : text;
}
...
function renderInline(text: string, keyPrefix = ""): React.ReactNode {
  ...
  while ((m = re.exec(text)) !== null) {
    if (m.index > last)
      parts.push(<React.Fragment key={`${keyPrefix}t${idx++}`}>{renderPlainWithLinks(text.slice(last, m.index), `${keyPrefix}tp${idx}`)}</React.Fragment>);
    if (m[3]) {
      parts.push(
        <img key={`${keyPrefix}img${idx++}`} src={normalizeScreenshotUrl(m[3])} alt={m[2] || ""} loading="lazy"
          style={{ maxWidth: "100%", borderRadius: "8px", margin: "4px 0" }} />
      );
    } else if (m[5]) {
      if (isLocalServerFileHref(m[5])) {
        parts.push(
          <LocalFileArtifactLink key={`${keyPrefix}a${idx++}`} href={m[5]}>
            {m[4]}
          </LocalFileArtifactLink>
        );
    ...
    else if (m[8] && isLocalServerFileHref(m[8]))
      parts.push(
        <LocalFileArtifactLink key={`${keyPrefix}c${idx++}`} href={m[8]}>
          {normalizeLocalFileHref(m[8]).split("/").pop() || "서버 파일"}
        </LocalFileArtifactLink>
      );
    else if (m[8])
      parts.push(
  ...
  if (last < text.length)
    parts.push(<React.Fragment key={`${keyPrefix}t${idx++}`}>{renderPlainWithLinks(text.slice(last), `${keyPrefix}tp${idx}`)}</React.Fragment>);
  return parts.length ? parts : text;
}
