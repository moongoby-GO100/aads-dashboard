...
export function useChatSSE() {
  ...
  // B2-FIX: streaming_status 3초 폴링 — SSE 끊김 시 just_completed 감지
  ...
  const sendMessage = useCallback(
    async (
      ...
      // B2-FIX: streaming_status 3초 폴링 시작 — SSE 끊김 시 just_completed 자동 감지
      stopStatusPoll();
      statusPollRef.current = setInterval(async () => {
        if (activeSessionRef.current !== sessionId) { stopStatusPoll(); return; }
        try {
          const ss = await chatApi.getStreamingStatus(sessionId);
          if (ss.just_completed) {
            stopStatusPoll();
            // just_completed 감지 → 메시지 재조회하여 최종 응답 반영
            const msgs = await chatApi.getMessages(sessionId, 50, 0);
            ...
            if (lastAI) {
              ...
              completeStream(sessionId, lastAI.content);
              onDone?.(lastAI.content, {
                modelUsed: lastAI.model_used || null,
                inputTokens: lastAI.input_tokens || null,
                outputTokens: lastAI.output_tokens || null,
                costUsd: lastAI.cost_usd || null,
      ...
      const doStream = async (): Promise<void> => {
        ...
        try {
            ...
            for (const event of events) {
              for (const line of event.split("\n")) {
                ...
                if (chunk.type === "delta" && chunk.content) {
                  ...
                  if (timeoutRef.current) clearTimeout(timeoutRef.current);
                  stopStatusPoll(); // B2-FIX: 정상 완료 시 폴링 중지
          ...
          stopStatusPoll(); // B2-FIX: 에러 시 폴링 중지
