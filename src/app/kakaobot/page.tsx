...
const menuCards = [
  { href: "/kakaobot/contacts", icon: "👥", title: "연락처 관리", desc: "친구·가족·거래처 연락처와 관계를 관리합니다", color: "var(--accent)" },
...
export default function KakaoBotDashboard() {
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "var(--bg-primary)" }}>
      <Header title="KakaoBot 대시보드" />
      <div className="flex-1 p-3 md:p-6 overflow-auto">

        {/* 상단 통계 */}
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
          {[
            { label: "연락처", value: stats?.total_contacts, color: "var(--accent)" },
        ...
        {/* 기능 카드 그리드 */}
        <h2 className="text-sm font-semibold mb-3" style={{ color: "var(--text-primary)" }}>기능 메뉴</h2>
        ...
        <div className="mt-6 rounded-xl p-5" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
          <h2 className="text-sm font-semibold mb-3" style={{ color: "var(--text-primary)" }}>💡 빠른 시작 가이드</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-start gap-3">
              <span className="text-lg shrink-0 w-8 h-8 flex items-center justify-center rounded-full" style={{ background: "var(--bg-hover)" }}>1</span>
              <div>
                <p className="text-xs font-semibold mb-0.5" style={{ color: "var(--text-primary)" }}>연락처 등록</p>
                <p className="text-xs" style={{ color: "var(--text-secondary)" }}>발송 대상의 이름, 관계, 기념일을 등록합니다</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <span className="text-lg shrink-0 w-8 h-8 flex items-center justify-center rounded-full" style={{ background: "var(--bg-hover)" }}>2</span>
              <div>
                <p className="text-xs font-semibold mb-0.5" style={{ color: "var(--text-primary)" }}>템플릿 선택 or AI 생성</p>
                <p className="text-xs" style={{ color: "var(--text-secondary)" }}>준비된 템플릿을 쓰거나 AI가 문구를 만들어줍니다</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <span className="text-lg shrink-0 w-8 h-8 flex items-center justify-center rounded-full" style={{ background: "var(--bg-hover)" }}>3</span>
              <div>
                <p className="text-xs font-semibold mb-0.5" style={{ color: "var(--text-primary)" }}>예약 발송</p>
                <p className="text-xs" style={{ color: "var(--text-secondary)" }}>원하는 시간에 자동으로 카카오톡이 발송됩니다</p>
              </div>
            </div>
          </div>
        </div>
