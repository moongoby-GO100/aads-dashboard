...
const ChatSidebar = memo(function ChatSidebar(props: ChatSidebarProps) {
  const {
    ...
    createSession, deleteSession, deleteWorkspace, deletingWorkspaceId, setShowAddProject, theme, toggleTheme,
    tagFilter, setTagFilter, allTags,
    workspaceLoadError, onRetryWorkspaceLoad,
  } = props;

  const showLeftSidebar = screenSize === "desktop" ? leftOpen : mobileOverlay === "sidebar";
  const workspaceDeleteStyle = (isActive: boolean, isDeleting: boolean): CSSProperties => ({
    width: "24px",
    height: "24px",
    display: "inline-flex",
    alignItems: "center",
    ...
    border: "none",
    borderRadius: "5px",
    background: isActive ? "rgba(239,68,68,0.16)" : "var(--ct-hover)",
    color: isActive ? "#fff" : "#ef4444",
    cursor: isDeleting ? "wait" : "pointer",
    fontSize: "13px",
    opacity: isDeleting ? 0.55 : 0.75,
    flexShrink: 0,
  });
            ...
            {workspaces.map((ws) => (
              <div key={ws.id} style={{ marginBottom: "4px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                  ...
                  >
                    <span>{ws.icon || "📁"}</span>
                    <span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis" }}>{ws.name}</span>
                  ...
                  <button
                    ...
                    style={workspaceDeleteStyle(activeWs === ws.id, deletingWorkspaceId === ws.id)}
                    onMouseEnter={(e) => {
                      if (deletingWorkspaceId !== ws.id) e.currentTarget.style.opacity = "1";
                    }}
                    onMouseLeave={(e) => {
                      if (deletingWorkspaceId !== ws.id) e.currentTarget.style.opacity = "0.75";
                    }}
                  >
                        ...
                        >
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: "2px", paddingRight: "2px" }}>
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                window.open(`${window.location.origin}/chat/${encodeURIComponent(s.id)}`, "_blank");
                              }}
            ...
            {workspaces.length === 0 && (
              <div
                style={{
                  padding: "20px 10px",
                  fontSize: "12px",
                  color: workspaceLoadError ? "var(--ct-warn, #f59e0b)" : "var(--ct-text2)",
                  textAlign: "center",
                }}
              >
                {workspaceLoadError ? (
                  <>
                    <div>{workspaceLoadError}</div>
