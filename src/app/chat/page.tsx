...
function interruptedBadgeLabel(message: ChatMessage): string {
  ...
  if (message.intent === "interrupted_partial") return "임시 저장됨";
  return "이어가기 가능";
}
...
function mergeServerMessagesPreservingLocal(
  ...
  // 사라져 보인다. 중복 placeholder는 최신 앵커 하나에 내용만 합쳐서 버블 리마운트를 막는다.
  let seenPh = false;
    ...
    if (merged[i].intent === "streaming_placeholder" || isStreamingPlaceholderMessage(merged[i])) {
      if (seenPh) {
        if (hasMeaningfulDisplayContent(merged[i])) {
          const targetIndex = merged.findIndex((message, index) =>
            index > i && (message.intent === "streaming_placeholder" || isStreamingPlaceholderMessage(message))
          );
          if (targetIndex >= 0) {
            const currentContent = merged[targetIndex].content || "";
            const duplicateContent = merged[i].content || "";
            merged[targetIndex] = {
              ...merged[targetIndex],
              content: duplicateContent.length > currentContent.length ? duplicateContent : currentContent,
              thinking_summary: merged[targetIndex].thinking_summary || merged[i].thinking_summary,
              thought_summary: merged[targetIndex].thought_summary || merged[i].thought_summary,
              tools_called: merged[targetIndex].tools_called || merged[i].tools_called,
              has_tools: Boolean(merged[targetIndex].has_tools) || Boolean(merged[i].has_tools),
              tool_count: merged[targetIndex].tool_count ?? merged[i].tool_count,
              tool_names: merged[targetIndex].tool_names ?? merged[i].tool_names,
            };
          }
        }
        merged.splice(i, 1);
          ...
          >
            <span style={{ display: "inline-flex", alignItems: "center", flexWrap: "wrap", gap: "4px" }}>
              {placeholderStatus ? (
                ...
                <span style={{ display: "inline-flex", alignItems: "center", gap: "3px", padding: "1px 6px", borderRadius: "8px", fontSize: "10px", fontWeight: 600, background: "rgba(59,130,246,0.08)", color: "#3b82f6", border: "1px solid rgba(59,130,246,0.20)" }}>{interruptedBadgeLabel(msg)}</span>
...
export default function ChatPage() {
  ...
  const setMessages = useCallback((updater: React.SetStateAction<ChatMessage[]>) => {
    const previous = messagesSnapshotRef.current;
  ...
  if (!chatViewportControllerRef.current) {
    chatViewportControllerRef.current = new ChatViewportController(
      createDomChatViewportAdapter(() => messagesContainerRef.current),
      {
        onFollowModeChange: setChatFollowMode,
        onUnreadCountChange: setViewportUnreadCount,
      },
    );
  }
  const chatViewportController = chatViewportControllerRef.current;
  useLayoutEffect(() => {
    chatViewportController.resetSession(activeSession?.id || null);
  }, [activeSession?.id, chatViewportController]);
  ...
  const jumpToLatestMessage = useCallback(() => {
    chatViewportController.jumpToLatest();
  }, [chatViewportController]);
  const settleScrollAfterMessageMerge = useCallback(() => {
    chatViewportController.settleAfterMessageChange(
      streamingRef.current || waitingBgRef.current,
      true,
    );
  }, [chatViewportController]);
  const captureMessageViewportAnchor = useCallback((): MessageViewportAnchor | null => {
    return chatViewportController.captureAnchor();
  }, [chatViewportController]);
  const restoreMessageViewportAnchor = useCallback((anchor: MessageViewportAnchor | null) => {
    if (!anchor) return;
    chatViewportController.restoreSessionAnchor(anchor);
    chatViewportController.commitPendingIntent();
  }, [chatViewportController]);
  const setMessagesPreservingViewport = useCallback((
    updater: React.SetStateAction<ChatMessage[]>,
    reason: ChatViewportReason = "message-commit",
  ) => {
    // Compute before React's updater: the updater remains replay-safe and DOM-free in Strict Mode.
    const previous = messagesSnapshotRef.current;
    const next = typeof updater === "function"
      ? (updater as (value: ChatMessage[]) => ChatMessage[])(previous)
      : updater;
  ...
  useLayoutEffect(() => {
    // The controller consumes a committed intent once synchronously and once in
    // the next frame. React state updaters never schedule DOM work.
    chatViewportController.commitPendingIntent();
  }, [chatViewportController, messages]);
    ...
    setMessagesPreservingViewport((prev) => {
      ...
      const resetPrev = prev
        ...
        .map((m) => {
          ...
          return {
            ...m,
            content: savedPartial?.content || m.content || "",
            intent: "streaming_placeholder",
            model_used: "streaming",
  ...
  }, [activeWs, cacheWorkspaceSessions, setMessages]);
  ...
  useEffect(() => {
    ...
    const markUserScrollIntent = () => {
      chatViewportController.markUserGesture();
    };
    ...
    const restoreIfUnexpectedTopReset = (source: "scroll" | "mutation") => {
      if (!isInitialLoadRef.current && chatViewportController.restoreUnexpectedTopReset()) {
    ...
    const handleScroll = () => {
      ...
      chatViewportController.recordScroll(chatViewportController.hasActiveGesture);
    };
    let mutationAuditFrame = 0;
    let resizeAuditFrame = 0;
    ...
    return () => {
      ...
      if (resizeAuditFrame) cancelAnimationFrame(resizeAuditFrame);
  ...
  useLayoutEffect(() => {
    ...
    if (isInitialLoadRef.current) {
      ...
      chatViewportController.requestBottom(true, "initial");
      // PERF: ResizeObserver로 DOM 변화 감지 (setInterval 50ms → 이벤트 기반)
      const observer = new ResizeObserver(() => {
        if (!isInitialLoadRef.current) return;
        chatViewportController.requestBottom(false, "initial");
      });
      observer.observe(container);
      const releaseInitialScrollLock = () => {
        observer.disconnect();
        isInitialLoadRef.current = false;
        chatViewportController.recordScroll(false);
      };
  ...
  }, [chatViewportController, messages, restoreMessageViewportAnchor]); // streamBuf 의존성 제거!

  // 스트리밍 중 스크롤 (200ms interval, near-bottom일 때만, streamBuf 의존성 제거로 렌더 감소)
  useEffect(() => {
    if (!streaming) return;
    const iv = setInterval(() => {
      chatViewportController.requestBottom(false, "message-commit");
    }, 300);
    return () => clearInterval(iv);
  }, [chatViewportController, streaming]);
  ...
  const sendMessage = useCallback(async function sendMessage(queuedContent?: string, _unused?: undefined, retryCount?: number, _existingMsgId?: string, _existingIdempotencyKey?: string) {
    ...
    isInitialLoadRef.current = false;
    chatViewportController.noteUserSend();
...
> 프롬프트: ${imgPrompt}`
      ...
      if (activeSessionObjRef.current?.id) {
        chatApi<{ queued: boolean; message?: string }>(`/chat/sessions/${activeSessionObjRef.current.id}/interrupt`, {
          ...
          if (!res?.queued) {
            ...
            window.setTimeout(() => {
              void sendMessage(interruptContent, undefined, undefined, interruptLocalId);
            }, 0);
            return;
          }
    ...
    if (userMsg.id.startsWith("tmp-") || userMsg.id.startsWith("interrupt-")) {
      localQuestionEchoIdsRef.current.add(userMsg.id);
    }
    ...
    if (_optimisticPending) {
      ...
      setMessagesPreservingViewport(prev => {
        ...
        if (hasActivePlaceholder) {
          return reconcileMessagesForActiveStreaming(prev, {
            sessionId: sessionId!,
            executionId: currentExecutionIdRef.current,
            partialContent: streamBufRef.current || bgPartialContent || "분석 중...",
          });
        }
        return [
          ...freezeStreamingPlaceholders(prev, streamBufRef.current || bgPartialContent),
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if ((statusCode === 502 || statusCode === 503 || statusCode === 504) && (retryCount || 0) < INITIAL_GATEWAY_RETRY_LIMIT) {
          ...
          setStreaming(true);
          setStreamBuf(streamBufRef.current || bgPartialContentRef.current || "분석 중...");
          setToolStatus("응답 작성 중...");
        ...
        for (const parsedEvent of directEventStream.push(value)) {
          ...
          try {
            ...
            if (ev.type === "partial_preserved" && ev.message) {
              ...
              _resetDrain();
              full = normalizedMessageContent(ev.message as ChatMessage);
              setStreamBuf(full);
              setBgPartialContent(full);
              setToolStatus("응답 작성 중...");
              continue;
            }
      ...
      >
        ...
        {chatFollowMode === "manual" && messages.length > 0 && (
          <button
            ...
            title={`${viewportUnreadCount > 0 ? `새 메시지 ${viewportUnreadCount}개 · ` : ""}최신 메시지로 이동하고 자동 스크롤 다시 켜기`}
            aria-label={`${viewportUnreadCount > 0 ? `새 메시지 ${viewportUnreadCount}개 · ` : ""}최신 메시지로 이동하고 자동 스크롤 다시 켜기`}
            style={{
              position: "absolute",
              left: "50%",
              bottom: "12px",
          ...
          >
            ↓ {viewportUnreadCount > 0 ? `새 메시지 ${viewportUnreadCount}개 · 최신으로` : "최신으로"}
          </button>
        )}
        </div>
