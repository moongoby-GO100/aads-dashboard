...
export default function NodeDetailPanel({ node, loading, onGenerateIdeas, onGenerateCounter, onExpandNode, onUpdateContent, onDeleteNode }: Props) {
            ...
            <div style={{ marginLeft: "auto", display: "flex", gap: "6px" }}>
              {onUpdateContent && (
                <button
                  ...
                  disabled={loading}
                  style={{ padding: "4px 10px", borderRadius: "4px", border: "1px solid var(--border)", background: "transparent", color: "var(--text-secondary)", fontSize: "11px", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.5 : 1 }}
                >
                  ✏️ 수정
                </button>
              )}
              {onDeleteNode && node.nodeType !== "topic" && (
                <button
                  onClick={() => onDeleteNode(node.id)}
                  disabled={loading}
                  style={{ padding: "4px 10px", borderRadius: "4px", border: "1px solid #f87171", background: "transparent", color: "#f87171", fontSize: "11px", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.5 : 1 }}
                >
                  삭제
                </button>
              )}
